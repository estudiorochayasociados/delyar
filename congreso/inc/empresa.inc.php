...
if (isset($_POST["enviar"])) {
	if (!empty($_POST["empresa"]) && !empty($_POST["cargo"]) && !empty($_POST["nombre"]) && !empty($_POST["apellido"]) && !empty($_POST["localidad"]) && !empty($_POST["provincia"]) && !empty($_POST["telefono"]) && !empty($_POST["email"])) {
		...
		if($resultado){
			...
			Muchas gracias<br/>";
			$mensaje_a = "
			Hay un nuevo inscripto y se cargó exitosamente.<br/>Los datos del inscripto son los siguientes:<br/>
			<br/>
			<p style='padding-left:30px'>
				<b>Tipo: </b>" . strtoupper($cat) . "<br/>
				<b>Empresa: </b>" .  strtoupper($empresa) . "<br/>
				<b>Cargo/Puesto: </b>" .  strtoupper($cargo) . "<br/><
...
<form method="post" style="display:<?php echo $dis ?>"  id="form" onsubmit="$('#submit').val('CARGANDO INSCRIPCIÓN...');$('#cargando').show()">
	<h1 style="font-weight: bold;color:#fff;text-shadow:0px 5px 5px #333">Inscripción para empresas</h1>
	<br/>
	<center class="col-lg-8 col-lg-offset-2">
		<label class="col-lg-6">Nombre de la Empresa:
			<br/>
			<input type="text" name="empresa" placeholder="Nombre de la Empresa" value="<?php echo (isset($_POST["empresa"]) ? $_POST["empresa"] : '') ?>" style="width: 100%" required />
		</label>
		<label class="col-lg-6 ">Cargo en la Empresa:
			<br/>
			<input type="text" name="cargo" placeholder="Cargo en la Empresa" value="<?php echo (isset($_POST["cargo"]) ? $_POST["cargo"] : '') ?>" style="width: 100%" required/>
		</label>
